...
ROOT::Math::PxPyPzEVector Sum4Vec(const ROOT::VecOps::RVec<double> &pt,
...
Eigen::TensorFixedSize<int, Eigen::Sizes<2>>
prefsrLeptons(const ROOT::VecOps::RVec<int> &status,
  ...
  for (std::size_t i = 0; i < ngenparts; ++i) {
    ...
    // particles in the event, and in case of radiation from taus and their
    // decay products)
  ...
  if (nphotos == 2) {
    ...
    throw std::range_error("Expected to find 2 pre-FSR leptons, but found " +
                           std::to_string(nall) + ", " +
...
helicity_tensor csAngularMoments(const CSVars &csvars,
  ...
  const helicity_tensor moments =
      original_weight * (scales * angular + offsets);
...
scale_tensor_t makeScaleTensor(const Vec_f &scale_weights, double thres) {
  ...
  //[0] is mur=0.5 muf=0.5; [1] is mur=0.5 muf=1; [2] is mur=0.5 muf=2; [3] is
  // mur=1 muf=0.5 ; [4] is mur=1 muf=1; [5] is mur=1 muf=2; [6] is mur=2
  // muf=0.5; [7] is mur=2 muf=1 ; [8] is mur=2 muf=2)*

  // ordering of the tensor axes are mur, muf with elements ordered
  // 0.5, 1.0, 2.0 clip large weights when filling

  scale_tensor_t res;
  res(0, 0) =
      std::clamp<double>(scale_weights[0], -thres, thres); // mur=0.5 muf=0.5;
  res(0, 1) =
      std::clamp<double>(scale_weights[1], -thres, thres); // mur=0.5 muf=1.0;
  res(0, 2) =
      std::clamp<double>(scale_weights[2], -thres, thres); // mur=0.5 muf=2.0;
  res(1, 0) =
      std::clamp<double>(scale_weights[3], -thres, thres); // mur=1.0 muf=0.5;
  res(1, 1) =
      std::clamp<double>(scale_weights[4], -thres, thres); // mur=1.0 muf=1.0;
  res(1, 2) =
      std::clamp<double>(scale_weights[5], -thres, thres); // mur=1.0 muf=2.0;
  res(2, 0) =
      std::clamp<double>(scale_weights[6], -thres, thres); // mur=2.0 muf=0.5;
  res(2, 1) =
      std::clamp<double>(scale_weights[7], -thres, thres); // mur=2.0 muf=1.0;
  res(2, 2) =
      std::clamp<double>(scale_weights[8], -thres, thres); // mur=2.0 muf=2.0;

  return res;
}

using helicity_scale_tensor_t =
    Eigen::TensorFixedSize<double, Eigen::Sizes<NHELICITY, 3, 3>>;
using helicity_tensor_t =
    Eigen::TensorFixedSize<double, Eigen::Sizes<NHELICITY>>;
using helicity_helicity_tensor_t =
    Eigen::TensorFixedSize<double, Eigen::Sizes<NHELICITY, NHELICITY>>;

helicity_scale_tensor_t
makeHelicityMomentScaleTensor(const CSVars &csvars,
                              const scale_tensor_t &scale_tensor,
                              double original_weight = 1.0) {
  ...
  Eigen::TensorFixedSize<double, Eigen::Sizes<nhelicity, 1, 1>> moments =
      csAngularMoments(csvars).reshape(broadcasthelicities);

  return original_weight *
         scale_tensor.reshape(reshapescale).broadcast(broadcasthelicities) *
         moments.broadcast(broadcastscales);
}

helicity_helicity_tensor_t
makeHelicityMomentHelicityTensor(const CSVars &csvars,
                                 double original_weight = 1.0) {
  ...
  Eigen::TensorFixedSize<double, Eigen::Sizes<nhelicity, 1>> moments =
      csAngularMoments(csvars).reshape(broadcasthels);
  helicity_helicity_tensor_t hel_hel_tensor =
      original_weight *
      moments.reshape(reshapehelicities).broadcast(broadcasthels) *
      moments.broadcast(broadcasthelicities);
  ...
  for (int i = 0; i < NHELICITY; ++i) {
    for (int j = 0; j < NHELICITY; ++j) {
      if (i != j) {
        ...
        hel_hel_tensor(i, i) = original_weight * moments(i);
    }
  }
  return hel_hel_tensor;
}

template <Eigen::Index Npdfs> class makeHelicityMomentPdfTensor {
public:
  makeHelicityMomentPdfTensor() {}

  using helicity_pdf_tensor_t =
      Eigen::TensorFixedSize<double, Eigen::Sizes<NHELICITY, Npdfs>>;
  using pdf_tensor_t = Eigen::TensorFixedSize<double, Eigen::Sizes<Npdfs>>;

  auto operator()(const CSVars &csvars, const pdf_tensor_t &pdf_tensor,
                  double original_weight = 1.0) {
    ...
    helicity_pdf_tensor =
        original_weight * moments.broadcast(broadcastpdf) *
        pdf_tensor.reshape(reshapepdf).broadcast(broadcasthelicities);
...
ewLeptons(const ROOT::VecOps::RVec<int> &status,
          const ROOT::VecOps::RVec<int> &statusFlags,
          const ROOT::VecOps::RVec<int> &pdgId,
  ...
  for (std::size_t i = 0; i < ngenparts; ++i) {
    ...
    const bool is_selected =
        is_lepton && (is_status1 || is_tau) && is_prompt && is_fromHardProcess;
  ...
  if (nleptons < 2) {
    throw std::range_error("Expected to find at least 2 prompt bare leptons "
                           "from hard process, but found " +
                           std::to_string(nleptons));
  }
...
ROOT::VecOps::RVec<ROOT::Math::PxPyPzEVector>
ewPhotons(const ROOT::VecOps::RVec<int> &status,
          const ROOT::VecOps::RVec<int> &statusFlags,
          const ROOT::VecOps::RVec<int> &pdgId,
          const ROOT::VecOps::RVec<double> &pt,
          const ROOT::VecOps::RVec<double> &eta,
...
ewGenVPhos(const ROOT::VecOps::RVec<PxPyPzEVector> &leptons,
           const ROOT::VecOps::RVec<PxPyPzEVector> &photons) {
...
int selectGenPart(const ROOT::VecOps::RVec<int> &status,
                  const ROOT::VecOps::RVec<int> &pdgId, const int absPdgIdMin,
                  const int absPdgIdMax, const int statusMin,
                  const int statusMax, bool require = true) {
  ...
  for (std::size_t i = 0; i < ngenparts; ++i) {
    ...
    if (iabsPdgId >= absPdgIdMin && iabsPdgId <= absPdgIdMax &&
        istatus >= statusMin && istatus <= statusMax && istatus > selstatus) {
      selidx = i;
      selstatus = istatus;
    }
  }
  ...
  if (require and selidx == -1) {
    throw std::runtime_error(
        "Expected to find a gen particle matching the criteria, but did not.");
  }
