...
def RecoilCalibrationHelper(fIn, args):
    ...
    helper = ROOT.wrem.RecoilCalibrationHelper[nstat](
        fIn, "base_transform", args.recoilUnc
    )
    return helper, nstat
...
def VPTReweightHelper(fIn):
    ...
    helper = ROOT.wrem.VPTReweightHelper(
        js["vpt_bins"], js["weights"], min(js["vpt_bins"]), max(js["vpt_bins"])
    )
    return helper
...
class Recoil:
    ...
    def __init__(self, pu_type, args, flavor="mu"):
        ...
        if self.flavor == "mumu" or self.flavor == "mu":
            self.vpt_reweight_helper_mc_data = VPTReweightHelper(
                f"{common.data_dir}/recoil/{pu_type}_{self.met}/vptrw_mc_data_mumu.json"
            )
            recoil_tflite = (
                f"{common.data_dir}/recoil/{pu_type}_{self.met}/model_mc_data.tflite"
            )
        else:
            self.vpt_reweight_helper_mc_data = VPTReweightHelper(
                f"{common.data_dir}/recoil/{pu_type}_{self.met}/vptrw_mc_data_ee.json"
            )
            recoil_tflite = (
                f"{common.data_dir}/recoil/{pu_type}_{self.met}/model_mc_data_ee.tflite"
            )

        self.recoilHelper, self.nstat = RecoilCalibrationHelper(recoil_tflite, args)
        self.recoil_syst_bkg_para = RecoilCalibrationUncertaintyHelper(
            recoil_tflite, "syst_bkg_para", args
        )
        self.recoil_syst_bkg_perp = RecoilCalibrationUncertaintyHelper(
            recoil_tflite, "syst_bkg_perp", args
        )
        ...
        self.axis_MET_pt = hist.axis.Regular(
            200, 0, 200, name="recoil_MET_pt", underflow=False
        )
        ...
        self.axis_lep_pt = hist.axis.Regular(
            100, 0.0, 100.0, name="pt", underflow=False
        )

        self.axis_run_no = hist.axis.Variable(
            list(range(277770, 284045, 1)) + [284045], name="run_no"
        )

        self.axis_recoil_magn = hist.axis.Regular(
            300, 0, 300, name="recoil_magn", underflow=False
        )
        self.axis_recoil_para_qt = hist.axis.Regular(
            400, -200, 200, name="recoil_para_qt"
        )
        self.axis_recoil_para = hist.axis.Regular(400, -300, 100, name="recoil_para")
        ...
        self.axis_mt = hist.axis.Regular(200, 0.0, 200.0, name="mt", underflow=False)
        ...
        self.axis_qt = hist.axis.Regular(200, 0, 200, name="qt", underflow=False)
        self.axis_qt_gen = hist.axis.Regular(
            200, 0, 200, name="qt_gen", underflow=False
        )
        self.axis_dphi = hist.axis.Regular(
            30, 0.0, np.pi, name="dphi", underflow=False, overflow=False
        )
        self.axis_res_ratio = hist.axis.Regular(
            20000, 0, 2, name="res", underflow=False, overflow=False
        )
        self.axis_res_diff = hist.axis.Regular(
        ...
        self.axis_recoil_2d_para = hist.axis.Regular(
            40, -10, 10, name="axis_recoil_2d_para", underflow=False, overflow=False
        )
        self.axis_recoil_2d_perp = hist.axis.Regular(
            40, -10, 10, name="axis_recoil_2d_perp", underflow=False, overflow=False
        )
    ...
    def add_histo(
        ...
        if self.isW and with_fakes:
            if "mt" in [
            ...
            ]:  # remove "mt" from axes_fakerate in case mT is in cols/axes
                ...
                cols_fakerate = [
                    col_mt if x == "transverseMass" else x for x in self.cols_fakerate
                ]  # replace transverseMass with correct one
                axes_fakerate = self.axes_fakerate
            self.results.append(
                self.df.HistoBoost(
                    name, axes + axes_fakerate, cols + cols_fakerate + [nominal_weight]
                )
            )
        else:
    ...
    def recoil_Z(self, df, results, dataset, datasets_to_apply, leps_uncorr, leps_corr):
        ...
        self.df = self.df.Define(
            "lep_uncorr_pt", f"wrem::Vec_d{{ {leps_uncorr[0]}, {leps_uncorr[4]} }}"
        )
        self.df = self.df.Define(
            "lep_uncorr_eta", f"wrem::Vec_d{{ {leps_uncorr[1]}, {leps_uncorr[5]} }}"
        )
        self.df = self.df.Define(
            "lep_uncorr_phi", f"wrem::Vec_d{{ {leps_uncorr[2]}, {leps_uncorr[6]} }}"
        )
        self.df = self.df.Define(
            "lep_uncorr_charge", f"wrem::Vec_i{{ {leps_uncorr[3]}, {leps_uncorr[7]} }}"
        )
    ...
    def setup_MET(self):
        ...
        elif self.met == "RawPFMET":
            met_pt, met_phi = "RawMET_pt", "RawMET_phi"
        elif self.met == "DeepMETReso":
            met_pt, met_phi = "DeepMETResolutionTune_pt", "DeepMETResolutionTune_phi"
        elif self.met == "DeepMETPVRobust" and self.pu_type == "highPU":
            met_pt, met_phi = "DeepMETPVRobust_pt", "DeepMETPVRobust_phi"
        elif self.met == "DeepMETPVRobustNoPUPPI" and self.pu_type == "highPU":
            met_pt, met_phi = "DeepMETPVRobustNoPUPPI_pt", "DeepMETPVRobustNoPUPPI_phi"
        elif self.met == "DeepMETResp":
            met_pt, met_phi = "DeepMETResponseTune_pt", "DeepMETResponseTune_phi"
        else:
            logger.warning(
                f"MET definition {self.met} not found, fall back to RawPFMET"
            )
            met_pt, met_phi = "RawMET_pt", "RawMET_phi"

        # uncorrected MET
        self.df = self.df.Define(
            "met_uncorr_pt", f"return (std::isfinite({met_pt})) ? {met_pt} : 10000"
        )
        self.df = self.df.Define(
            "met_uncorr_phi", f"return (std::isfinite({met_phi})) ? {met_phi} : 0"
        )
        ...
        self.df = self.df.Define(
            "met_corr_lep",
            f"wrem::met_lepton_correction(met_uncorr_pt, met_uncorr_phi, lep_uncorr_pt, lep_uncorr_phi, lep_corr_pt, lep_corr_phi)",
        )
        self.df = self.df.Define("met_corr_lep_pt", "met_corr_lep[0]")
        self.df = self.df.Define("met_corr_lep_phi", "met_corr_lep[1]")
        self.df = self.df.Define(
            "met_corr_lep_x", "met_corr_lep_pt*cos(met_corr_lep_phi)"
        )
        self.df = self.df.Define(
            "met_corr_lep_y", "met_corr_lep_pt*sin(met_corr_lep_phi)"
        )
        ...
        if self.met_xy_helper_data != None and self.met_xy_helper_mc != None:
            self.df = self.df.Define(
                "met_corr_xy",
                (
                    self.met_xy_helper_data
                    if self.dataset.is_data
                    else self.met_xy_helper_mc
                ),
                ["met_corr_lep_pt", "met_corr_lep_phi", "npvs"],
            )
        ...
        self.add_histo(
            "met_corr_lep_x_npv",
            ["npvs", "met_corr_lep_x"],
            [self.axis_npv, self.axis_MET_xy],
        )
        self.add_histo(
            "met_corr_lep_y_npv",
            ["npvs", "met_corr_lep_y"],
        ...
        self.df = self.df.Define(
            "lep_pt_uncorr_minus_corr", "lep_uncorr_pt-lep_corr_pt"
        )
        self.df = self.df.Define(
            "lep_phi_uncorr_minus_corr", "lep_uncorr_phi-lep_corr_phi"
        )
        self.df = self.df.Define(
            "met_pt_uncorr_over_corr_lep", "met_uncorr_pt/met_corr_lep_pt"
        )
        self.df = self.df.Define(
            "met_pt_uncorr_minus_corr_lep", "met_uncorr_pt-met_corr_lep_pt"
        )
        self.df = self.df.Define(
            "met_phi_uncorr_minus_corr_lep", "met_uncorr_phi-met_corr_lep_phi"
        )

        self.add_histo(
            "lep_pt_uncorr_over_corr",
            ["lep_pt_uncorr_over_corr"],
            [self.axis_res_ratio],
        )
        self.add_histo(
            "lep_pt_uncorr_minus_corr",
            ["lep_pt_uncorr_minus_corr"],
            [self.axis_res_diff],
        )
        self.add_histo(
            "lep_phi_uncorr_minus_corr",
            ["lep_phi_uncorr_minus_corr"],
            [self.axis_res_diff],
        )

        self.add_histo(
            "met_pt_uncorr_over_corr_lep",
            ["met_pt_uncorr_over_corr_lep"],
            [self.axis_res_ratio],
        )
        self.add_histo(
            "met_pt_uncorr_minus_corr_lep",
            ["met_pt_uncorr_minus_corr_lep"],
            [self.axis_res_diff],
        )
        self.add_histo(
            "met_phi_uncorr_minus_corr_lep",
            ["met_phi_uncorr_minus_corr_lep"],
            [self.axis_res_diff],
        )
    ...
    def recoil_vars_plots_Z(self, rec_corr, nominal_weight="nominal_weight", suffix=""):
        ...
        if not self.df.HasColumn(f"recoil_{rec_corr}_magn"):
            self.df = self.df.Define(
                f"recoil_{rec_corr}_magn",
                f"std::hypot(recoil_{rec_corr}_para, recoil_{rec_corr}_perp)",
            )
            self.df = self.df.Define(
                f"met_{rec_corr}_wlike",
                f"wrem::get_met_wlike(lep_nontrg_pt, lep_nontrg_phi, met_{rec_corr}_pt, met_{rec_corr}_phi)",
            )
            self.df = self.df.Define(
                f"met_{rec_corr}_pt_wlike", f"met_{rec_corr}_wlike.Mod()"
            )
            self.df = self.df.Define(
                f"met_{rec_corr}_phi_wlike", f"met_{rec_corr}_wlike.Phi()"
            )
            self.df = self.df.Define(
                f"mt_{rec_corr}",
                f"wrem::get_mt_wlike(lep_trg_pt, lep_trg_phi, met_{rec_corr}_wlike)",
            )
            self.df = self.df.Define(
                f"dphi_{rec_corr}",
                f"std::abs(wrem::deltaPhi(v_phi, met_{rec_corr}_phi))",
            )
            self.df = self.df.Define(
                f"dphi_{rec_corr}_wlike",
                f"std::abs(wrem::deltaPhi(lep_trg_phi, met_{rec_corr}_phi_wlike))",
            )
        ...
        self.add_histo(
            f"recoil_{rec_corr}_magn{suffix}",
            [f"recoil_{rec_corr}_magn"],
            [self.axis_recoil_magn],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"recoil_{rec_corr}_para_qt{suffix}",
            [f"recoil_{rec_corr}_para_qt"],
            [self.axis_recoil_para_qt],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"recoil_{rec_corr}_para{suffix}",
            [f"recoil_{rec_corr}_para"],
            [self.axis_recoil_para],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"recoil_{rec_corr}_perp{suffix}",
            [f"recoil_{rec_corr}_perp"],
            [self.axis_recoil_perp],
            nominal_weight=nominal_weight,
        )

        self.add_histo(
            f"mt_{rec_corr}{suffix}",
            [f"mt_{rec_corr}"],
            [self.axis_mt],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"met_{rec_corr}_pt{suffix}",
            [f"met_{rec_corr}_pt"],
            [self.axis_MET_pt],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"met_{rec_corr}_phi{suffix}",
            [f"met_{rec_corr}_phi"],
            [self.axis_MET_phi],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"met_{rec_corr}_pt_wlike{suffix}",
            [f"met_{rec_corr}_pt_wlike"],
            [self.axis_MET_pt],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"met_{rec_corr}_phi_wlike{suffix}",
            [f"met_{rec_corr}_phi_wlike"],
            [self.axis_MET_phi],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"dphi_{rec_corr}{suffix}",
            [f"dphi_{rec_corr}"],
            [self.axis_dphi],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"dphi_{rec_corr}_wlike{suffix}",
            [f"dphi_{rec_corr}_wlike"],
            [self.axis_dphi],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"met_{rec_corr}_x{suffix}",
            [f"met_{rec_corr}_x"],
            [self.axis_MET_xy],
            nominal_weight=nominal_weight,
        )
        self.add_histo(
            f"met_{rec_corr}_y{suffix}",
            [f"met_{rec_corr}_y"],
            [self.axis_MET_xy],
            nominal_weight=nominal_weight,
        )
    ...
    def recoil_vars_plots_W(self, rec_corr, nominal_weight="nominal_weight", suffix=""):
        ...
        if not self.df.HasColumn(f"recoil_{rec_corr}_magn"):
            self.df = self.df.Define(
                f"recoil_{rec_corr}_magn",
                f"wrem::compute_recoil_from_met_and_lepton(met_{rec_corr}_pt, met_{rec_corr}_phi, lep_trg_pt, lep_trg_phi)",
            )
            self.df = self.df.Define(
                f"mt_{rec_corr}",
                f"wrem::mt_2(lep_trg_pt, lep_trg_phi, met_{rec_corr}_pt, met_{rec_corr}_phi)",
            )
            self.df = self.df.Define(
                f"dphi_{rec_corr}",
                f"std::abs(wrem::deltaPhi(lep_trg_phi, met_{rec_corr}_phi))",
            )
        ...
        self.add_histo(
            f"lep_{rec_corr}_pt{suffix}",
            ["lep_trg_pt"],
            [self.axis_lep_pt_test],
            with_fakes=True,
            col_mt=f"mt_{rec_corr}",
        )
    ...
    def setup_recoil_Z(self):
        ...
        self.df = self.df.Define(
            "recoil_uncorr",
            "wrem::compute_recoil_from_met(met_uncorr_pt, met_uncorr_phi, lep_corr_pt, lep_corr_phi, v_pt, v_phi)",
        )
        ...
        self.df = self.df.Define(
            "recoil_corr_lep",
            "wrem::compute_recoil_from_met(met_corr_lep_pt, met_corr_lep_phi, lep_corr_pt, lep_corr_phi, v_pt, v_phi)",
        )
        ...
        self.df = self.df.Define(
            "recoil_corr_lep_para_qt", "recoil_corr_lep_para + v_pt"
        )

        # MET XY corrected recoil
        self.df = self.df.Define(
            "recoil_corr_xy",
            "wrem::compute_recoil_from_met(met_corr_xy_pt, met_corr_xy_phi, lep_corr_pt, lep_corr_phi, v_pt, v_phi)",
        )
        ...
        self.recoil_vars_plots_Z(
            "corr_xy", suffix="qtrw", nominal_weight="nominal_weight_vptrw_mc_data"
        )
        ...
        self.add_histo(
            "recoil_corr_xy_para_v_pt",
            ["v_pt", "recoil_corr_xy_para"],
            [self.axis_qt, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_para_qt_v_pt",
            ["v_pt", "recoil_corr_xy_para_qt"],
            [self.axis_qt, self.axis_recoil_para_qt],
        )
        self.add_histo(
            "recoil_corr_xy_perp_v_pt",
            ["v_pt", "recoil_corr_xy_perp"],
            [self.axis_qt, self.axis_recoil_perp],
        )

        self.add_histo(
            "recoil_corr_xy_para_npv",
            ["npvs", "recoil_corr_xy_para"],
            [self.axis_npv, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_para_qt_npv",
            ["npvs", "recoil_corr_xy_para_qt"],
            [self.axis_npv, self.axis_recoil_para_qt],
        )
        self.add_histo(
            "recoil_corr_xy_perp_npv",
            ["npvs", "recoil_corr_xy_perp"],
            [self.axis_npv, self.axis_recoil_perp],
        )

        # response vs npv
        self.add_histo(
            "recoil_corr_xy_para_qt_v_pt_npv",
            ["v_pt", "npvs", "recoil_corr_xy_para_qt"],
            [self.axis_qt, self.axis_npv, self.axis_recoil_para_qt],
        )
        self.add_histo(
            "recoil_corr_xy_para_v_pt_npv",
            ["v_pt", "npvs", "recoil_corr_xy_para"],
            [self.axis_qt, self.axis_npv, self.axis_recoil_para],
        )
        self.add_histo("v_gen_pt_npv", ["v_pt", "npvs"], [self.axis_qt, self.axis_npv])

        # recoil vs rapidity
        self.add_histo(
            "recoil_corr_xy_para_y",
            ["v_y", "recoil_corr_xy_para"],
            [self.axis_rapidity, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_perp_y",
            ["v_y", "recoil_corr_xy_perp"],
            [self.axis_rapidity, self.axis_recoil_perp],
        )

        self.add_histo(
            "recoil_corr_xy_para_sumEt",
            ["RawMET_sumEt", "recoil_corr_xy_para"],
            [self.axis_sumEt, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_perp_sumEt",
            ["RawMET_sumEt", "recoil_corr_xy_perp"],
            [self.axis_sumEt, self.axis_recoil_perp],
        )

        self.add_histo(
            "recoil_corr_xy_para_perp",
            ["recoil_corr_xy_para", "recoil_corr_xy_perp"],
            [self.axis_recoil_para, self.axis_recoil_perp],
        )
        ...
        self.add_histo(
            ...
            ["v_pt"],
            [self.axis_qt],
            nominal_weight="nominal_weight_vptrw_mc_data",
        )
    ...
    def auxHists(self):
        ...
        self.add_histo(
            "npv_RawMET_sumEt",
            ["npvs", "RawMET_sumEt"],
            [self.axis_npv, self.axis_sumEt],
        )
        self.add_histo(
            "qT_sumEt", ["v_pt", "RawMET_sumEt"], [self.axis_qt, self.axis_sumEt]
        )

        self.add_histo(
            "recoil_corr_xy_para_qT_njets",
            ["njets", "recoil_corr_xy_para_qt"],
            [self.axis_njets, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_perp_njets",
            ["njets", "recoil_corr_xy_perp"],
            [self.axis_njets, self.axis_recoil_perp],
        )
        ...
        if self.dataset.name in self.datasets_to_apply:
            self.add_histo(
                "mt_corr_xy_qt",
                ["mt_corr_xy", "v_gen_pt"],
                [self.axis_mt, self.axis_qt],
            )

        # run-dependent stuff for data
        if self.dataset.is_data and False:
            self.add_histo(
                "njets_runNo", ["njets", "run"], [self.axis_njets, self.axis_run_no]
            )
            self.add_histo(
                "RawMET_sumEt_runNo",
                ["RawMET_sumEt", "run"],
                [self.axis_sumEt, self.axis_run_no],
            )

            self.add_histo(
                "recoil_uncorr_para_runNo",
                ["recoil_uncorr_para", "run"],
                [self.axis_recoil_para, self.axis_run_no],
            )
            self.add_histo(
                "recoil_uncorr_perp_runNo",
                ["recoil_uncorr_perp", "run"],
                [self.axis_recoil_perp, self.axis_run_no],
            )
            self.add_histo(
                "recoil_corr_lep_para_runNo",
                ["recoil_corr_lep_para", "run"],
                [self.axis_recoil_para, self.axis_run_no],
            )
            self.add_histo(
                "recoil_corr_lep_perp_runNo",
                ["recoil_corr_lep_perp", "run"],
                [self.axis_recoil_perp, self.axis_run_no],
            )
            self.add_histo(
                "recoil_corr_xy_para_runNo",
                ["recoil_corr_xy_para", "run"],
                [self.axis_recoil_para, self.axis_run_no],
            )
            self.add_histo(
                "recoil_corr_xy_perp_runNo",
                ["recoil_corr_xy_perp", "run"],
                [self.axis_recoil_perp, self.axis_run_no],
            )
            ...
            self.add_histo(
                "METx_corr_lep_runNo",
                ["METx_corr_lep", "run"],
                [self.axis_MET_xy, self.axis_run_no],
            )
            self.add_histo(
                "METy_corr_lep_runNo",
                ["METy_corr_lep", "run"],
            ...
            self.add_histo(
                "npv_runNo", ["npvs", "run"], [self.axis_npv, self.axis_run_no]
            )
    ...
    def setup_gen_reco_vars_Z(self):
        ...
        self.df = self.df.Define(
            "lep0_mom4",
            f"ROOT::Math::PtEtaPhiMVector(lep_corr_pt[0], lep_corr_eta[0], lep_corr_phi[0], {mass})",
        )
        self.df = self.df.Define(
            "lep1_mom4",
            f"ROOT::Math::PtEtaPhiMVector(lep_corr_pt[1], lep_corr_eta[1], lep_corr_phi[1], {mass})",
        )
        self.df = self.df.Define(
            "vmom4",
            "ROOT::Math::PxPyPzEVector(lep0_mom4)+ROOT::Math::PxPyPzEVector(lep1_mom4)",
        )
        ...
        if self.dataset.name in self.datasets_to_apply:
            ...
            def gen_res_vars(suffix, gen_pt, gen_phi):
                self.df = self.df.Define(
                    f"reco_over_gen_v_pt{suffix}", f"v_pt/{gen_pt}"
                )
                self.df = self.df.Define(
                    f"reco_minus_gen_v_pt{suffix}", f"v_pt-{gen_pt}"
                )
                self.df = self.df.Define(
                    f"reco_minus_gen_v_phi{suffix}", f"v_phi-{gen_phi}"
                )
                if self.storeHists:
                    self.add_histo(f"v_gen_pt{suffix}", [gen_pt], [self.axis_qt])
                    self.add_histo(
                        f"reco_over_gen_v_pt{suffix}",
                        [f"reco_over_gen_v_pt{suffix}"],
                        [self.axis_res_ratio],
                    )
                    self.add_histo(
                        f"reco_minus_gen_v_pt{suffix}",
                        [f"reco_minus_gen_v_pt{suffix}"],
                        [self.axis_res_diff],
                    )
                    self.add_histo(
                        f"reco_minus_gen_v_phi{suffix}",
                        [f"reco_minus_gen_v_phi{suffix}"],
                        [self.axis_res_diff],
                    )
                    self.add_histo(
                        f"v_gen_reco_pt{suffix}",
                        [gen_pt, "v_pt"],
                        [self.axis_qt_gen, self.axis_qt],
                    )
            ...
            self.df = self.df.Define(
                "trg_lep_mom4",
                f"ROOT::Math::PtEtaPhiMVector(lep_trg_pt, lep_trg_eta, lep_trg_phi, wrem::muon_mass)",
            )
            self.df = self.df.Define(
                "proxy_gen_prefsr",
                f"wrem::proxy_gen_v(genl, genlanti, trg_lep_mom4, lep_trg_charge)",
            )
            ...
            self.df = self.df.Define(
                "proxy_gen_postfsr",
                f"wrem::proxy_gen_v(lepGen, antilepGen, trg_lep_mom4, lep_trg_charge)",
            )
            ...
            if self.vpt_reweight_helper_mc_data != None:
                self.df = self.df.Define(
                    "vpt_weight_mc_data", self.vpt_reweight_helper_mc_data, ["v_pt"]
                )
            else:
                self.df = self.df.Define("vpt_weight_mc_data", "1.0")
            self.df = self.df.Define(
                "nominal_weight_vptrw_mc_data", "nominal_weight*vpt_weight_mc_data"
            )
    ...
    def setup_gen_reco_vars_W(self):
        if self.dataset.name in self.datasets_to_apply:
            ...
            if self.vpt_reweight_helper_mc_data != None:
                self.df = self.df.Define(
                    "vpt_weight_mc_data", self.vpt_reweight_helper_mc_data, ["v_gen_pt"]
                )
            else:
                self.df = self.df.Define("vpt_weight_mc_data", "1.0")
            self.df = self.df.Define(
    ...
    def setup_recoil_gen(self):
        ...
        self.df = self.df.Define(
            "recoil_corr_xy_para_qt_gen", "recoil_corr_xy_para_gen + v_gen_pt"
        )
        ...
        self.add_histo(
            ...
            ["v_gen_pt", "recoil_corr_xy_para_gen"],
            [self.axis_qt, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_para_qt_gen_v_gen_pt",
            ["v_gen_pt", "recoil_corr_xy_para_qt_gen"],
            [self.axis_qt, self.axis_recoil_para_qt],
        )
        self.add_histo(
            "recoil_corr_xy_perp_gen_v_gen_pt",
            ["v_gen_pt", "recoil_corr_xy_perp_gen"],
            [self.axis_qt, self.axis_recoil_perp],
        )

        self.add_histo(
            "recoil_corr_xy_para_gen_npv",
            ["npvs", "recoil_corr_xy_para_gen"],
            [self.axis_npv, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_para_qt_gen_npv",
            ["npvs", "recoil_corr_xy_para_qt_gen"],
            [self.axis_npv, self.axis_recoil_para_qt],
        )
        self.add_histo(
            "recoil_corr_xy_perp_gen_npv",
            ["npvs", "recoil_corr_xy_perp_gen"],
            [self.axis_npv, self.axis_recoil_perp],
        )

        # response vs npv
        self.add_histo(
            "recoil_corr_xy_para_gen_v_gen_pt_npv",
            ["v_gen_pt", "npvs", "recoil_corr_xy_para_gen"],
            [self.axis_qt, self.axis_npv, self.axis_recoil_para],
        )
        self.add_histo(
            "recoil_corr_xy_para_qt_gen_v_gen_pt_npv",
            ["v_gen_pt", "npvs", "recoil_corr_xy_para_qt_gen"],
            [self.axis_qt, self.axis_npv, self.axis_recoil_para_qt],
        )
        self.add_histo(
            "recoil_corr_xy_perp_gen_v_gen_pt_npv",
            ["v_gen_pt", "npvs", "recoil_corr_xy_perp_gen"],
            [self.axis_qt, self.axis_npv, self.axis_recoil_perp],
        )
        self.add_histo(
            "v_gen_pt_npv", ["v_gen_pt", "npvs"], [self.axis_qt, self.axis_npv]
        )
    ...
    def apply_recoil_Z(self):
        ...
        if self.dataset.name in self.datasets_to_apply:
            ...
            if doGEN:
                if self.recoilHelper != None:
                    ...
                    self.df = self.df.Define(
                        "recoil_corr_rec_para_qt", "recoil_corr.ut_para_corr(0)"
                    )
                    self.df = self.df.Define(
                        "recoil_corr_rec_perp", "recoil_corr.ut_perp_corr(0)"
                    )
                else:
                    self.df = self.df.Alias(
                        "recoil_corr_rec_para_qt", "recoil_corr_xy_para_qt_gen"
                    )
                    self.df = self.df.Alias(
                        "recoil_corr_rec_perp", "recoil_corr_xy_perp_gen"
                    )
                self.df = self.df.Define(
                    "recoil_corr_rec_para", "recoil_corr_rec_para_qt - v_gen_pt"
                )
                ...
                self.df = self.df.Define(
                    "met_corr_rec_x", "met_corr_rec_pt*cos(met_corr_rec_phi)"
                )
                self.df = self.df.Define(
                ...
                if self.recoilHelper != None:
                    self.df = self.df.Define(
                        "recoil_corr",
                        self.recoilHelper,
                        ["v_pt", "recoil_corr_xy_para_qt", "recoil_corr_xy_perp"],
                    )
                    self.df = self.df.Define(
                        "recoil_corr_rec_para_qt", "recoil_corr.ut_para_corr(0)"
                    )
                    ...
                    self.df = self.df.Alias(
                        "recoil_corr_rec_para_qt", "recoil_corr_xy_para_qt"
                    )
                    self.df = self.df.Alias(
                        "recoil_corr_rec_perp", "recoil_corr_xy_perp"
                    )
                self.df = self.df.Define(
                    "recoil_corr_rec_para", "recoil_corr_rec_para_qt - v_pt"
                )

                self.df = self.df.Define(
                    "met_corr_rec",
                    "wrem::compute_met_from_recoil(recoil_corr_rec_para, recoil_corr_rec_perp, lep_corr_pt, lep_corr_phi, v_pt, v_phi)",
                )
                self.df = self.df.Define("met_corr_rec_pt", "met_corr_rec[0]")
                self.df = self.df.Define("met_corr_rec_phi", "met_corr_rec[1]")
                self.df = self.df.Define(
        ...
        self.recoil_vars_plots_Z(
            "corr_rec", suffix="qtrw", nominal_weight="nominal_weight_vptrw_mc_data"
        )
    ...
    def apply_recoil_W(self):
        ...
        if self.dataset.name in self.datasets_to_apply:
            ...
            if self.recoilHelper != None:
                ...
                self.df = self.df.Define(
                    "recoil_corr_rec_para_qt_gen", "recoil_corr.ut_para_corr(0)"
                )
                self.df = self.df.Define(
                    "recoil_corr_rec_perp_gen", "recoil_corr.ut_perp_corr(0)"
                )
            else:
                self.df = self.df.Alias(
                    "recoil_corr_rec_para_qt_gen", "recoil_corr_xy_para_qt_gen"
                )
                self.df = self.df.Alias(
                    "recoil_corr_rec_perp_gen", "recoil_corr_xy_perp_gen"
                )
            self.df = self.df.Define(
                "recoil_corr_rec_para_gen", "recoil_corr_rec_para_qt_gen - v_gen_pt"
            )
    ...
    def add_recoil_unc_Z(
        ...
        results.append(
            df.HistoBoost(
                f"{hName}_recoil_stat",
                axes,
                cols + [self.recoil_unc_stat_weights_with_nom],
                tensor_axes=[self.recoil_var_ax_stat],
        ...
        if self.nsyst > 0:
            results.append(
                df.HistoBoost(
                    f"{hName}_recoil_syst",
                    axes,
                    cols + [self.recoil_unc_syst_weights_with_nom],
                    tensor_axes=[self.recoil_var_ax_syst],
                    storage=storage_type,
                )
            )
        return df

    def add_recoil_unc_W(
        self,
        df,
        results,
        dataset,
        ...
        axes,
        hName,
        add_fakerate_cols=False,
        col_mt_orig="transverseMass",
        col_mt_target="transverseMass",
        storage_type=hist.storage.Double(),
    ):
        if not dataset.name in self.datasets_to_apply:
            return df
        ...
        if add_fakerate_cols:
            if "mt" in [
                ...
                cols_fakerate = [
                    col_mt_target if x == col_mt_orig else x for x in self.cols_fakerate
                ]  # replace transverseMass with correct one
                axes_fakerate = self.axes_fakerate
            axes_ = axes + axes_fakerate
            cols_ = cols + cols_fakerate
        else:
            axes_ = axes
            cols_ = cols
        ...
        self.results.append(
            df.HistoBoost(
                f"{hName}_recoil_stat",
                axes_,
                cols_ + [self.recoil_unc_stat_weights_with_nom],
                tensor_axes=[self.recoil_var_ax_stat],
                storage=storage_type,
            )
        )
        if self.nsyst > 0:
            self.results.append(
                df.HistoBoost(
                    f"{hName}_recoil_syst",
                    axes_,
                    cols_ + [self.recoil_unc_syst_weights_with_nom],
                    tensor_axes=[self.recoil_var_ax_syst],
                    storage=storage_type,
                )
            )
    ...
    def setup_recoil_Z_unc(self):
        ...
        self.df = self.df.Define(
            self.recoil_unc_stat_weights_with_nom,
            "auto res = recoil_unc_stat_weights; res = nominal_weight*res; return res;",
        )
        self.recoil_var_ax_stat = hist.axis.Integer(
            0, self.nstat, name="recoil_unc", underflow=False, overflow=False
        )
        ...
        if self.recoil_syst_bkg_para != None:
            self.df = self.df.Define(
                "recoil_syst_bkg_para_z",
                self.recoil_syst_bkg_para,
                ["v_pt", "recoil_corr_xy_para_qt"],
            )
            recoil_systs.append("recoil_syst_bkg_para_z")
        if self.recoil_syst_bkg_perp != None:
            self.df = self.df.Define(
                "recoil_syst_bkg_perp_z",
                self.recoil_syst_bkg_perp,
                ["v_pt", "recoil_corr_xy_perp"],
            )
            recoil_systs.append("recoil_syst_bkg_perp_z")
        ...
        if self.nsyst > 0:
            ...
            self.df = self.df.Define(
                self.recoil_unc_syst_weights_with_nom,
                f"wrem::concatWeights<{len(recoil_systs)}>(nominal_weight, {', '.join(recoil_systs)})",
            )
            self.recoil_var_ax_syst = hist.axis.Integer(
                0, self.nsyst, name="recoil_unc", underflow=False, overflow=False
            )
            for hName, col, ax in zip(hNames, cols, axes):
                self.df = self.add_recoil_unc_Z(
                    self.df, self.results, self.dataset, col, ax, hName
                )
    ...
    def setup_recoil_W_unc(self):
        ...
        for hName, col, ax in zip(hNames, cols, axes):
            self.df = self.add_recoil_unc_W(
                ...
                hName,
                col_mt_orig="transverseMass",
                col_mt_target="mt_corr_rec",
                add_fakerate_cols=True,
            )
