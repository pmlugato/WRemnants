...
def make_muon_efficiency_helpers_smooth(
    ...
    fin = root_io.safeOpenRootFile(filename)
    ...
    if len(eff_types_3D):
        if era not in ["2017", "2018"]:
            ...
            fileSF3D = f"{data_dir}/muonSF/{era}/smoothSF3D.pkl.lz4"

        if not os.path.isfile(fileSF3D):
            raise IOError(
                f"Couldn't read 3D SF file {fileSF3D}, make sure you have it."
            )
    ...
    for charge, charge_tag in charges.items():
        for (
            ...
            allEff_types
        ):  # should only loop on axis_eff_type_2D but must get syst in 2D to propagate to 3D later
            # for iso can use histogram for efficiency variation only in data (the only one we have for now)
            nameTag = (
                "nomiAndAlt_onlyDataVar"
                if (
                    isoEfficiencySmoothing
                    and any(x in eff_type for x in ["iso", "antiiso"])
                )
                else "nomiAndAlt"
            )
            ...
            if isoDefinition != "iso04vtxAgn" or era != "2016PostVFP":
                if eff_type == "antitrigger":
                    ...
                    if templateAnalysisArg == "wrem::AnalysisType::Dilepton":
                        logger.warning(
                            f"Substituting temporarily missing 2D histogram for 'antitrigger' with 'trigger'"
                        )
            ...
            hist_root = root_io.safeGetRootObject(fin, hist_name)
            ...
            if eff_type not in axis_eff_type_2D:
                ...
                if key not in sf_syst_from2D_for3D.keys():
                    ...
                    sf_syst_from2D_for3D[key] = hh.divideHists(
                        hist_hist[:, :, axis_nomiAlt_eff.size - 1],
                        hist_hist[:, :, 0],
                        createNew=True,
                    )
                    logger.debug(
                        f"Storing 2D eta-pt syst for {key} SF to propagate to 3D version"
                    )
            ...
            sf_syst_2D.view(flow=False)[
                :, :, axis_charge.index(charge), axis_eff_type_2D.index(eff_type), 0
            ...
            sf_syst_2D.view(flow=False)[
                :, :, axis_charge.index(charge), axis_eff_type_2D.index(eff_type), 1
            ] = hist_hist.view(flow=False)[:, :, axis_nomiAlt_eff.size - 1]
            for isyst in range(len(effSyst_decorrEtaEdges) - 1):
                ...
                sf_syst_2D.view(flow=False)[
                    ...
                    axis_eff_type_2D.index(eff_type),
                    2 + isyst,
                ] = hist_hist.view(flow=False)[:, :, 0]
                ...
                sf_syst_2D.view(flow=False)[
                    ...
                    indexEtaLow:indexEtaHigh, :, axis_nomiAlt_eff.size - 1
                ]
    ...
    if len(eff_types_3D):
        ...
        for charge, charge_tag in charges.items():
            for eff_type in axis_eff_type_3D:
                ...
                if sf_syst_3D is None:
                    ...
                    axis_eta_eff = cloneAxis(
                        hist_hist.axes[0], overflow=True, underflow=True
                    )
                    axis_pt_eff = cloneAxis(
                        hist_hist.axes[1], overflow=True, underflow=True
                    )
                    axis_ut_eff = cloneAxis(
                        hist_hist.axes[2], overflow=True, underflow=True
                    )
                    # ut axis is put last to keep same axes sorting as for 2D histograms created before
                    sf_syst_3D = hist.Hist(
                        axis_eta_eff,
                        axis_pt_eff,
                        axis_charge,
                        axis_eff_type_3D,
                ...
                sf_syst_3D.view(flow=False)[
                    ...
                    :,
                    axis_charge.index(charge),
                    axis_eff_type_3D.index(eff_type),
                    0,
                    :,
                ] = nominalLayer  # hist_hist.view(flow=False)[:,:,:,0]
                ...
                for isyst in range(len(effSyst_decorrEtaEdges) - 1):
                    ...
                    sf_syst_3D.view(flow=False)[
                        ...
                        indexEtaLow:indexEtaHigh, :, :
                    ]  # hist_hist.view(flow=False)[indexEtaLow:indexEtaHigh, :, :, 0]

        # set overflow and underflow eta-pt bins equal to adjacent bins
        sf_syst_3D.view(flow=True)[0, ...] = sf_syst_3D.view(flow=True)[1, ...]
        sf_syst_3D.view(flow=True)[axis_eta_eff.extent - 1, ...] = sf_syst_3D.view(
            flow=True
        )[axis_eta_eff.extent - 2, ...]
        sf_syst_3D.view(flow=True)[:, 0, ...] = sf_syst_3D.view(flow=True)[:, 1, ...]
        sf_syst_3D.view(flow=True)[:, axis_pt_eff.extent - 1, ...] = sf_syst_3D.view(
            flow=True
        )[:, axis_pt_eff.extent - 2, ...]
        sf_syst_3D.view(flow=True)[..., 0] = sf_syst_3D.view(flow=True)[..., 1]
        sf_syst_3D.view(flow=True)[..., axis_ut_eff.extent - 1] = sf_syst_3D.view(
            flow=True
        )[..., axis_ut_eff.extent - 2]
        ...
        helper = ROOT.wrem.muon_efficiency_smooth3D_helper[
            templateAnalysisArg, Nsyst, type(sf_syst_2D_pyroot), type(sf_syst_3D_pyroot)
        ](ROOT.std.move(sf_syst_2D_pyroot), ROOT.std.move(sf_syst_3D_pyroot))
        helper_syst = ROOT.wrem.muon_efficiency_smooth3D_helper_syst[
            templateAnalysisArg, Nsyst, type(sf_syst_2D_pyroot), type(sf_syst_3D_pyroot)
        ](helper)
    ...
    for effStatKey in effStat_manager.keys():
        ...
        for ic, (charge, charge_tag) in enumerate(charges.items()):
            for eff_type in effStat_manager[effStatKey]["axisLabels"]:
                ...
                if eff_type in chargeDependentSteps:
                    ...
                    chargeTag = "both"
                    if ic:
                        continue  # must continue after having set the charge axis
                ...
                if is3D:
                    ...
                    ]  # this is a 4D histogram, eta-pt-ut-var, with NO OVERFLOWS (unlike the standard 2D case where it comes from root)
                    nPtEigenBins = int(
                        hist_hist.axes[3].size - 1
                    )  # 1 nominal + N eigen variations, we need N here
                else:
                    nameTag = "nomiAndAlt"
                ...
                if nom_up_effStat_axis is None:
                    ...
                    effStat_manager[effStatKey]["nCharges"] = (
                        2 if eff_type in chargeDependentSteps else 1
                    )
                    nom_up_effStat_axis = hist.axis.Regular(
                        int(1 + nPtEigenBins),
                        -0.5,
                        0.5 + nPtEigenBins,
                        underflow=False,
                ...
                if effStat_manager[effStatKey]["boostHist"] is None:
                    ...
                    axis_ut_eff = (
                        ...
                        if is3D
                        else hist.axis.Regular(1, -1e6, 1e6, name="SF ut")
                    )
                    axis_eff_type = hist.axis.StrCategory(
        ...
        if smooth3D:
            if "sf_iso" in effStatKey:
                helper_stat = ROOT.wrem.muon_efficiency_smooth_helper_stat_iso_utDep[
                    ...
                    ncharges,
                    type(sf_stat_pyroot),
                ](ROOT.std.move(sf_stat_pyroot))
            else:
                helper_stat = ROOT.wrem.muon_efficiency_smooth_helper_stat_utDep[
                    templateAnalysisArg,
                    netabins,
                    nPtEigenBins,
                    ncharges,
            ...
            if "sf_iso" in effStatKey:
                helper_stat = ROOT.wrem.muon_efficiency_smooth_helper_stat_iso[
                    templateAnalysisArg,
                    netabins,
                    nPtEigenBins,
                    ncharges,
                ...
                helper_stat = ROOT.wrem.muon_efficiency_smooth_helper_stat[
                    templateAnalysisArg,
                    netabins,
                    nPtEigenBins,
                    ncharges,
        ...
        axis_ptEigen_eff_tensor = hist.axis.Integer(
            0,
            effStat_manager[effStatKey]["nPtEigenBins"],
            underflow=False,
            overflow=False,
            name="nPtEigenBins",
        )
    ...
    return (
        helper,
        helper_syst,
        {k: effStat_manager[k]["helper"] for k in effStat_manager.keys()},
    )


#### the following only deal with special additional systematics, no effStat
def make_muon_efficiency_helpers_smooth_altSyst(
    filename=data_dir + "/muonSF/allSmooth_GtoHout_vtxAgnIso_altBkg.root",
    era=None,
    what_analysis=ROOT.wrem.AnalysisType.Wmass,
    max_pt=np.inf,
    effStep=["reco", "tracking"],
):

    allEff_types = [effStep] if isinstance(effStep, str) else effStep[:]

    validEffSteps = ["reco", "tracking"]
    for es in allEff_types:
        if es not in validEffSteps:
            raise NotImplementedError(
                f"Step {es} not implemented for alternate background systematics"
            )
    ...
    for charge, charge_tag in charges.items():
        for eff_type in allEff_types:
            ...
            # extract nominal (first bin that is not underflow) and put in corresponding bin of destination
            sf_syst_2D.values(flow=False)[:, :, axis_charge.index(charge), 0] = (
                hist_hist.values(flow=False)[:, :, 0]
            )
            # extract syst (last bin except overflow) and put in corresponding bin of destination (bin 1 is the second bin because no underflow)
            sf_syst_2D.values(flow=False)[:, :, axis_charge.index(charge), 1] = (
                hist_hist.values(flow=False)[:, :, axis_nomiAlt_eff.size - 1]
            )
            for isyst in range(len(effSyst_decorrEtaEdges) - 1):
                # first copy the nominal
                sf_syst_2D.values(flow=False)[
                    :, :, axis_charge.index(charge), 2 + isyst
                ] = hist_hist.values(flow=False)[:, :, 0]
                # now update with actual syst all eta bins inside interval [effSyst_decorrEtaEdges[isyst], effSyst_decorrEtaEdges[isyst+1]]
                # add epsilon to ensure picking the bin on the right of the edge (for the right edge given by
                # effSyst_decorrEtaEdges[isyst+1]] the range selection in boost later on will stop at the left
                ...
                sf_syst_2D.values(flow=False)[
                    indexEtaLow:indexEtaHigh, :, axis_charge.index(charge), 2 + isyst
                ] = hist_hist.values(flow=False)[
                    indexEtaLow:indexEtaHigh, :, axis_nomiAlt_eff.size - 1
                ]

    # set overflow and underflow eta-pt bins equal to adjacent bins
    sf_syst_2D.values(flow=True)[0, ...] = sf_syst_2D.values(flow=True)[1, ...]
    sf_syst_2D.values(flow=True)[axis_eta_eff.extent - 1, ...] = sf_syst_2D.values(
        flow=True
    )[axis_eta_eff.extent - 2, ...]
    sf_syst_2D.values(flow=True)[:, 0, ...] = sf_syst_2D.values(flow=True)[:, 1, ...]
    sf_syst_2D.values(flow=True)[:, axis_pt_eff.extent - 1, ...] = sf_syst_2D.values(
        flow=True
    )[:, axis_pt_eff.extent - 2, ...]
    ...
    # case with only 2D histograms
    sf_syst_2D_pyroot = narf.hist_to_pyroot_boost(sf_syst_2D)
    helper_syst = ROOT.wrem.muon_efficiency_smooth_helper_syst_oneStep[
        templateAnalysisArg, Nsyst, type(sf_syst_2D_pyroot)
    ](ROOT.std.move(sf_syst_2D_pyroot))
    # define axis for syst variations with all steps
