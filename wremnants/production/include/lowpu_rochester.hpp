...
struct RocRes {
  ...
  double kSmear(double pt, double eta, TYPE type, double v, double u,
                int n) const;
...
public: // was private?
  ...
  double kGenSmear(double pt, double eta, double v, double u,
                   RocRes::TYPE TT = RocRes::Data, int s = 0, int m = 0) const;
  double kScaleMC(int Q, double pt, double eta, double phi, int s = 0,
                  int m = 0) const;

  double kScaleDT(int Q, double pt, double eta, double phi, int s = 0,
                  int m = 0) const;
  double kSpreadMC(int Q, double pt, double eta, double phi, double gt,
                   int s = 0, int m = 0) const;
  double kSmearMC(int Q, double pt, double eta, double phi, int n, double u,
                  int s = 0, int m = 0) const;

  double kScaleDTerror(int Q, double pt, double eta, double phi) const;
  double kSpreadMCerror(int Q, double pt, double eta, double phi,
                        double gt) const;
  double kSmearMCerror(int Q, double pt, double eta, double phi, int n,
                       double u) const;

  // old, should only be used with 2017v0
  double kScaleFromGenMC(int Q, double pt, double eta, double phi, int n,
                         double gt, double w, int s = 0, int m = 0) const;
  double kScaleAndSmearMC(int Q, double pt, double eta, double phi, int n,
                          double u, double w, int s = 0, int m = 0) const;
  double kScaleFromGenMCerror(int Q, double pt, double eta, double phi, int n,
                              double gt, double w) const;
  double kScaleAndSmearMCerror(int Q, double pt, double eta, double phi, int n,
...
RoccoR::RoccoR(std::string filename) { init(filename); }
    ...
    if (first4 == "NSET") {
      ...
      if (resol.empty()) {
        ...
        for (size_t ir = 0; ir < resol.size(); ++ir) {
          ...
          r.cb.resize(RTRK);
          for (auto i : {0, 1})
            r.nTrk[i].resize(RTRK + 1);
          for (auto i : {0, 1, 2})
            r.rsPar[i].resize(RTRK);
        }
      }
      auto &cp = rc.CP;
      for (TYPE T : {MC, DT}) {
      ...
      if (tag == "R") {
        ...
        ss >> type >> bin;
        for (int i = 0; i < RTRK + 1; ++i)
          ss >> resol[bin].nTrk[type][i];
      } else if (tag == "F") {
        ss >> type;
...
double RoccoR::kScaleDT(int Q, double pt, double eta, double phi, int s,
                        int m) const {
  int H = etaBin(eta);
  int F = phiBin(phi);
...
double RoccoR::kScaleMC(int Q, double pt, double eta, double phi, int s,
                        int m) const {
  int H = etaBin(eta);
  int F = phiBin(phi);
...
double RoccoR::kSpreadMC(int Q, double pt, double eta, double phi, double gt,
                         int s, int m) const {
...
double RoccoR::kSmearMC(int Q, double pt, double eta, double phi, int n,
                        double u, int s, int m) const {
  const auto &rc = RC[s][m];
  int H = etaBin(eta);
  int F = phiBin(phi);
  double k = 1.0 / (rc.CP[MC][H][F].M + Q * rc.CP[MC][H][F].A * pt);
...
double RoccoR::kScaleFromGenMC(int Q, double pt, double eta, double phi, int n,
                               double gt, double w, int s, int m) const {
  const auto &rc = RC[s][m];
  int H = etaBin(eta);
  int F = phiBin(phi);
  double k = 1.0 / (rc.CP[MC][H][F].M + Q * rc.CP[MC][H][F].A * pt);
...
double RoccoR::kScaleAndSmearMC(int Q, double pt, double eta, double phi, int n,
                                double u, double w, int s, int m) const {
  const auto &rc = RC[s][m];
  int H = etaBin(eta);
  int F = phiBin(phi);
  double k = 1.0 / (rc.CP[MC][H][F].M + Q * rc.CP[MC][H][F].A * pt);
...
double RoccoR::kGenSmear(double pt, double eta, double v, double u,
                         RocRes::TYPE TT, int s, int m) const {
  if (empty())
    return 1.0;
  return RC[s][m].RR.kSmear(pt, eta, TT, v, u);
}

template <typename T> double RoccoR::error(T f) const {
  double sum = 0;
...
double RoccoR::kSpreadMCerror(int Q, double pt, double eta, double phi,
                              double gt) const {
...
double RoccoR::kSmearMCerror(int Q, double pt, double eta, double phi, int n,
                             double u) const {
...
double RoccoR::kScaleFromGenMCerror(int Q, double pt, double eta, double phi,
                                    int n, double gt, double w) const {
  return error([this, Q, pt, eta, phi, n, gt, w](int s, int m) {
    return kScaleFromGenMC(Q, pt, eta, phi, n, gt, w, s, m);
  });
}

double RoccoR::kScaleAndSmearMCerror(int Q, double pt, double eta, double phi,
                                     int n, double u, double w) const {
  return error([this, Q, pt, eta, phi, n, u, w](int s, int m) {
    return kScaleAndSmearMC(Q, pt, eta, phi, n, u, w, s, m);
  });
}

// RoccoR * rochester = new
// RoccoR("wremnants/data/lowPU/rochester/RoccoR2017.txt"); //
// https://gitlab.cern.ch/akhukhun/roccor
RoccoR *rochester = new RoccoR(
    "wremnants-data/data/lowPU/rochester/RoccoR_lowPU_v0.txt"); // v0 version
                                                                // for lowPU
...
Vec_f applyRochesterMC(Vec_f pt, Vec_f eta, Vec_f phi, Vec_f ch, Vec_i gen_idx,
  ...
  for (unsigned int i = 0; i < size; ++i) {
    ...
    if (gen_idx.at(i) >= 0)
      res[i] = 1.0000 * pt[i] *
               rochester->kSpreadMC(ch[i], pt[i], eta[i], phi[i],
                                    gen_pt.at(gen_idx.at(i)), fluctuation, 0);
    else
      res[i] = 1.0000 * pt[i] *
...
Vec_f applyRochesterData(Vec_f pt, Vec_f eta, Vec_f phi, Vec_f ch,
  ...
  for (unsigned int i = 0; i < size; ++i) {
    ...
    res[i] = pt[i] *
             rochester->kScaleDT(ch[i], pt[i], eta[i], phi[i], fluctuation, 0);
  }
  return res;
}
