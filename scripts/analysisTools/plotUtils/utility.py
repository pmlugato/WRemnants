...
# }
colors_plots_ = {
    "Wmunu": ROOT.TColor.GetColor("#e42536"),
...
def common_plot_parser():
    ...
    parser.add_argument(
        ...
        default=112,
        type=int,
        help="Set palette: 55 is kRainbow, 112 is kViridis, 113 is kCividis, 87 is kLightTemperature",
    )
    parser.add_argument(
        "--invertPalette", action="store_true", help="Inverte color ordering in palette"
    )
    parser.add_argument(
        "--eosMount",
        dest="eoscp",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        choices=list(gatherProcesses_.keys()),
        help="Set list of processes to be gathered in the legend",
    )
    parser.add_argument(
        "--CMStext",
        type=str,
        ...
        help="Text for label without CMS",
    )
    return parser
...
def checkHistInFile(h, hname, fname, message=""):
    if not h:
        logger.error(
            "Error {msg}: I couldn't find histogram {h} in file {f}".format(
                msg=message, h=hname, f=fname
            )
        )
        quit()
...
def getMinMaxHisto(
    ...
    if dim == 1:
        ...
        nbins = (h.GetNbinsX() + 2) * (h.GetNbinsY() + 2)
    elif dim == 3:
        nbins = (h.GetNbinsX() + 2) * (h.GetNbinsY() + 2) * (h.GetNbinsZ() + 2)
    else:
    ...
    for ibin in range(1, nbins + 1):
        if ibin <= nBinMin or ibin >= nBinMax:
            continue
        if excludeUnderflow and h.IsBinUnderflow(ibin):
            continue
        if excludeOverflow and h.IsBinOverflow(ibin):
            continue
        tmpmax = h.GetBinContent(ibin)
        tmpmin = h.GetBinContent(ibin)
        if excludeEmpty and tmpmin == 0.0:
            continue
        if excludeMin != None and tmpmin <= excludeMin:
            continue
        if excludeMax != None and tmpmax >= excludeMax:
            continue
        if firstValidBin < 0:
    ...
    if dim == 1:
        for ix in range(1, h.GetNbinsX() + 1):
            if retmin > h.GetBinContent(ix):
                if excludeMin != None:
                    if h.GetBinContent(ix) > excludeMin:
        ...
        for ix in range(1, h.GetNbinsX() + 1):
            for iy in range(1, h.GetNbinsY() + 1):
                if retmin > h.GetBinContent(ix, iy):
                    if excludeMin != None:
                        if h.GetBinContent(ix, iy) > excludeMin:
            ...
            for iy in range(1, h.GetNbinsY() + 1):
                for iz in range(1, h.GetNbinsZ() + 1):
                    if retmin > h.GetBinContent(ix, iy, iz):
                        if excludeMin != None:
                            if h.GetBinContent(ix, iy, iz) > excludeMin:
        ...
        raise RuntimeError(
            "Error in getMinimumTH(): unsupported histogram's dimension (%d)" % dim
        )
    ...
    if dim == 1:
        for ix in range(1, h.GetNbinsX() + 1):
            if retmax < h.GetBinContent(ix):
                if excludeMax != None:
                    if h.GetBinContent(ix) < excludeMax:
        ...
        for ix in range(1, h.GetNbinsX() + 1):
            for iy in range(1, h.GetNbinsY() + 1):
                if retmax < h.GetBinContent(ix, iy):
                    if excludeMax != None:
                        if h.GetBinContent(ix, iy) < excludeMax:
            ...
            for iy in range(1, h.GetNbinsY() + 1):
                for iz in range(1, h.GetNbinsZ() + 1):
                    if retmax < h.GetBinContent(ix, iy, iz):
                        if excludeMax != None:
                            if h.GetBinContent(ix, iy, iz) < excludeMax:
        ...
        raise RuntimeError(
            "Error in getMaximumTH(): unsupported histogram's dimension (%d)" % dim
        )
...
def fillTH2fromTH2part(
    ...
    xoffset=0,
    yoffset=0,
    fillWithValuePlusError=False,
    scaleError=1.0,  # fill with binContent + scaleError*binError (negative scaleError to subtract)
    fillWithError=False,
    useRelativeError=False,
    ratioValForZeroAtDen=0.0,
):
    ...
    for ix in range(xbinLow, 1 + xbinHigh):
        for iy in range(ybinLow, 1 + ybinHigh):
            if fillWithValuePlusError:
                content = h2in.GetBinContent(
                    ix + xoffset, iy + yoffset
                ) + scaleError * h2in.GetBinError(ix + xoffset, iy + yoffset)
                ...
                if useRelativeError:
                    if h2in.GetBinContent(ix + xoffset, iy + yoffset) != 0.0:
                        content = content / h2in.GetBinContent(
                            ix + xoffset, iy + yoffset
                        )
                    else:
...
def fillTH2fromTH3zrange(h2, h3, zbinLow=1, zbinHigh=1):
    for ix in range(1, 1 + h2.GetNbinsX()):
        for iy in range(1, 1 + h2.GetNbinsY()):
            error = ROOT.Double(0)
            h2.SetBinContent(
                ix, iy, h3.IntegralAndError(ix, ix, iy, iy, zbinLow, zbinHigh, error)
            )
...
def fillTHNplus1fromTHn(thnp1, thn, nbinLow=-1, nbinHigh=-1):
    ...
    nbinsTHn = (
        thn.GetNbins()
    )  # for TH3 or lower one should use GetNcells() to emulate this, but one can get a proper THn from a TH3 to use consistent methods
...
def multiplyByHistoWith1ptBin(h, h1bin):
    ...
    for ix in range(1, 1 + h.GetNbinsX()):
        for iy in range(1, 1 + h.GetNbinsY()):
            h.SetBinContent(
                ix, iy, h.GetBinContent(ix, iy) * h1bin.GetBinContent(ix, 1)
            )
    ...
    for ix in range(1, 1 + h.GetNbinsX()):
        for iy in range(1, 1 + h.GetNbinsY()):
            ...
            if neglectUncSecond:
                ...
                unc = math.sqrt(
                    hContent * hContent * hlessUnc * hlessUnc
                    + hlessContent * hlessContent * hUnc * hUnc
                )
...
def scaleTH2byOtherTH2(h, hother, scaleUncertainty=True, divide=False):
    ...
    for ix in range(1, 1 + h.GetNbinsX()):
        ...
        xbin = sorted((1, xbin, maxXbinOther))[
            1
        ]  # creative way to clamp, faster than max(1, min(val, maxVal)) syntax
        for iy in range(1, 1 + h.GetNbinsY()):
...
def getTH2morePtBins(h2, newname, nPt):
    xedges = [
        round(h2.GetXaxis().GetBinLowEdge(i), 2) for i in range(1, 2 + h2.GetNbinsX())
    ]
    xarr = array("d", xedges)
    h2new = ROOT.TH2D(
        newname,
        ...
        xarr,
        nPt,
        h2.GetYaxis().GetBinLowEdge(1),
        h2.GetYaxis().GetBinLowEdge(1 + h2.GetNbinsY()),
    )
...
def createPlotDirAndCopyPhp(outdir, eoscp=True):
    ...
    outdir = output_tools.make_plot_dir(
        outdir, outfolder=None, eoscp=eoscp, allowCreateLocalFolder=True
    )
...
def getAxisRangeFromUser(axisNameTmp="", separator="::", rangeSeparator=","):
...
def drawCorrelationPlot(
    ...
    if rebinFactorX:
        if isinstance(rebinFactorX, int):
            h2D_tmp.RebinX(rebinFactorX)
        else:
            h2D_tmp.RebinX(
                len(rebinFactorX) - 1, "", array("d", rebinFactorX)
            )  # case in which rebinFactorX is a list of bin edges

    if rebinFactorY:
        if isinstance(rebinFactorY, int):
            h2D_tmp.RebinY(rebinFactorY)
        else:
            h2D_tmp.RebinY(
                len(rebinFactorY) - 1, "", array("d", rebinFactorY)
            )  # case in which rebinFactorX is a list of bin edges
    ...
    if palette > 0:
        ROOT.gStyle.SetPalette(
            palette
        )  # 55:raibow palette ; 57: kBird (blue to yellow, default) ; 107 kVisibleSpectrum ; 77 kDarkRainBow
    ...
    if not smoothPlot:
        ...
        for iBinX in range(1, 1 + h2D.GetNbinsX()):
            for iBinY in range(1, 1 + h2D.GetNbinsY()):
                h2DGraph.SetPoint(
                    nPoint,
                    h2D.GetXaxis().GetBinCenter(iBinX),
                    h2D.GetYaxis().GetBinCenter(iBinY),
                    h2D.GetBinContent(iBinX, iBinY),
                )
                nPoint += 1
    ...
    h2DPlot.GetXaxis().SetTitleOffset(
        0.95
    )  # 1.1 goes outside sometimes, maybe depends on root version or canvas width
    ...
    if setXAxisRangeFromUser:
        h2DPlot.GetXaxis().SetRangeUser(xmin, xmax)
    if setYAxisRangeFromUser:
        h2DPlot.GetYaxis().SetRangeUser(ymin, ymax)
    if setZAxisRangeFromUser:
        h2DPlot.GetZaxis().SetRangeUser(zmin, zmax)
    ...
    if drawProfileX:
        leg.AddEntry(
            h2DProfile, "Correlation = %.2f" % h2DPlot.GetCorrelationFactor(), ""
        )
        nLegEntries += 1
    if nLegEntries == 0:
        leg.SetFillStyle(0)
    ...
    if draw_both0_noLog1_onlyLog2 == 0 or draw_both0_noLog1_onlyLog2 == 2:
        ...
        for ext in ["png", "pdf"]:
            canvas.SaveAs(
                "{od}/{cn}_logZ.{ext}".format(od=outdir, cn=canvasName, ext=ext)
            )
        canvas.SetLogz(0)
...
def drawSingleTH1(
    ...
    yRatioAxisName, setRatioYAxisRangeFromUser, yminRatio, ymaxRatio = (
        getAxisRangeFromUser(labelRatioTmp)
    )
    ...
    if lowerPanelHeight:
        ...
        pad2 = ROOT.TPad("pad2", "pad2", 0, 0.0, 1, 0.9)
    ...
    h1err.SetFillStyle(
        1001
    )  # 3001 is better than 3002 for pdf, while 3002 is perfect for png
    ...
    if len(drawVertLines):
        ...
        sliceLabelOffset = 6.0 if "#eta" in textForLines[0] else 6.0
        ...
        if nLines:
            for i in range(0, nLines):  # we need nptBins texts
                if nLines < 31 or not (i % 5):  # only print 5 labels
                    ...
                    bintext.DrawLatex(
                        etarange * i + etarange / sliceLabelOffset,
                        ytext,
                        textForLines[i],
                    )
    ...
    if not skipLumi:
        if leftMargin > 0.1:
            ...
            if lumi != None:
                latCMS.DrawLatex(0.91, 0.95, "%s fb^{-1} (13 TeV)" % lumi)
            else:
                latCMS.DrawLatex(0.96, 0.95, "(13 TeV)")
...
def drawSingleTH1withFit(
    ...
    f2 = ROOT.TF1(
        "f2", pol2_scaled, xMaxFit, h1.GetXaxis().GetBinLowEdge(1 + h1.GetNbinsX()), 3
    )
    ...
    f2.Draw("L SAME")
    f2pol1 = ROOT.TF1(
        "f2pol1", pol1_scaled, 30, h1.GetXaxis().GetBinLowEdge(1 + h1.GetNbinsX()), 2
    )
...
def drawNTH1(
    ...
    if ymin == ymax == 0.0:
        ...
        ymax = -9999.9
        for h in hists:
            if h.GetBinContent(h.GetMaximumBin()) > ymax:
                ymax = h.GetBinContent(h.GetMaximumBin())
            if h.GetBinContent(h.GetMinimumBin()) < ymin:
                ymin = h.GetBinContent(h.GetMinimumBin())
        if ymin < 0:
            ymin = 0
    ...
    for il, le in enumerate(legEntries):
        leg.AddEntry(
            hists[il],
            le,
            firstHistogramStyle if il == 0 else "L" if onlyLineColor else "FL",
        )
    leg.Draw("same")
    canvas.RedrawAxis("sameaxis")
    ...
    if lowerPanelHeight:
        ...
        if len(hists) == 2 and not useMultiHistRatioOption:
            ...
            if not setRatioYAxisRangeFromUser:
                logger.debug(
                    f"drawNTH1(): setting y axis in ratio panel to this range: {newymin}, {newymax}"
                )
                frame.GetYaxis().SetRangeUser(newymin, newymax)
            pad2.RedrawAxis("sameaxis")
...
def drawDataAndMC(
    ...
    if len(drawVertLines):
        ...
        for i in range(1, nptBins):  # do not need line at canvas borders
            # vertline.DrawLine(offsetXaxisHist+etarange*i,0,offsetXaxisHist+etarange*i,canvas.GetUymax())
            vertline.DrawLine(
                etarange * i - offsetXaxisHist, 0, etarange * i - offsetXaxisHist, ymax
            )
        if len(textForLines):
            for i in range(0, len(textForLines)):  # we need nptBins texts
                ...
                ytext = 0.6 * ymax
                bintext.DrawLatex(
                    etarange * i + etarange / sliceLabelOffset, ytext, textForLines[i]
                )
    ...
    if drawLumiLatex:
        ...
        if lumi != None:
            ...
            latCMS.DrawLatex(0.8, 0.95, "(13 TeV)" % lumi)
    else:
        if lumi != None:
            CMS_lumi(canvas, lumi, True, False)
        else:
    ...
    if lowerPanelHeight:
        ...
        if invertRatio:
            if histMCpartialUnc == None:
                ...
                ratio.SetLineWidth(
                    1
                )  # make it smaller when it is drawn on top of something
        else:
            if histMCpartialUnc == None:
                den.SetFillColor(ROOT.kGray + 1)
        ...
        line = ROOT.TF1(
            "horiz_line",
            "0" if useDifferenceInLowerPanel else "1",
            ratio.GetXaxis().GetBinLowEdge(1),
            ratio.GetXaxis().GetBinLowEdge(ratio.GetNbinsX() + 1),
        )
        line.SetLineWidth(1)
        ...
        if invertRatio:
            ...
            if histMCpartialUnc != None:
                ratioline.Draw(
                    "HIST same"
                )  # to draw the line inside the band for the expected
            den.Draw("EPsame")
        else:
            ratio.Draw("EPsame")
...
def drawTH1dataMCstack(
    ...
    ROOT.TGaxis.SetExponentOffset(
        -0.025 if (wideCanvas or leftMargin < 0.1) else -0.09, 0.0, "y"
    )
    ROOT.TH1.SetDefaultSumw2()
    adjustSettings_CMS_lumi()
    ...
    labelX, setXAxisRangeFromUser, xmin, xmax = getAxisRangeFromUser(labelXtmp)
    labelY, setYAxisRangeFromUser, ymin, ymax = getAxisRangeFromUser(labelYtmp)
    labelRatioY, setRatioYAxisRangeFromUser, yminRatio, ymaxRatio = (
        getAxisRangeFromUser(ratioPadYaxisNameTmp)
    )
    ...
    stackCopy = (
        dummystack.GetStack().Last()
    )  # used to make ratioplot without affecting the plot and setting maximum
    ...
    if setYAxisRangeFromUser:
        ...
        ymaxBackup = (
            max(
                ...
                stackCopy.GetBinContent(stackCopy.GetMaximumBin()),
            )
            * yRangeScaleFactor
        )
    ...
    if len(drawVertLines):
        ...
        nLines = len(textForLines)
        if nLines:
            offsetText = etarange / (6.0 if len(textForLines) > 15 else 4.0)
            for i in range(0, nLines):  # we need nptBins texts
                if nLines < 31 or not (i % 5):  # only print 5 labels
                    bintext.DrawLatex(
                        etarange * i + offsetText,
                        textYheightOffset * ymaxBackup,
                        textForLines[i],
                    )
    ...
    if wideCanvas:
        ...
        latCMS.DrawLatex(leftMargin, 0.95, "#bf{CMS} #it{Preliminary}")
        if lumi != None:
            latCMS.DrawLatex(0.915, 0.95, "%s fb^{-1} (13 TeV)" % lumi)
        else:
            latCMS.DrawLatex(0.96, 0.95, "(13 TeV)")
    else:
        if not skipLumi:
            if not drawLumiLatex:
                ...
                latCMS.DrawLatex(
                    leftMargin if xcmsText < 0 else xcmsText,
                    0.95,
    ...
    if draw_both0_noLog1_onlyLog2 != 1:
        if labelY == "a.u.":
            h1.GetYaxis().SetRangeUser(
                max(0.0001, h1.GetMinimum() * 0.8),
                h1.GetBinContent(h1.GetMaximumBin()) * 100,
            )
        else:
            h1.GetYaxis().SetRangeUser(
                max(0.1, h1.GetMinimum() * 0.8),
                h1.GetBinContent(h1.GetMaximumBin()) * 1000,
            )
        canvas.SetLogy()
        canvas.RedrawAxis("sameaxis")
        canvas.SaveAs(outdir + canvasName + "_logY.png")
...
def drawCheckTheoryBand(
    ...
    if ratio2.InheritsFrom("TH1"):
        ...
        for i in range(den_noerr.GetNbinsX()):
            ratio2.SetPoint(
                i,
                den_noerr.GetBinCenter(i + 1),
                ratio2.Eval(den_noerr.GetBinCenter(i + 1)) / h1.GetBinContent(i + 1),
            )
    ...
    if ratio3.InheritsFrom("TH1"):
        ...
        for i in range(den_noerr.GetNbinsX()):
            ratio3.SetPoint(
                i,
                den_noerr.GetBinCenter(i + 1),
                ratio3.Eval(den_noerr.GetBinCenter(i + 1)) / h1.GetBinContent(i + 1),
            )
    ...
    line = ROOT.TF1(
        "horiz_line",
        "1",
        ratio1.GetXaxis().GetBinLowEdge(1),
        ratio1.GetXaxis().GetBinLowEdge(ratio1.GetNbinsX() + 1),
    )
...
def drawXsecAndTheoryband(
    h1,
    h2,  # h1 is data, h2 is total uncertainty band
    labelXtmp="xaxis",
    labelYtmp="yaxis",
    canvasName="default",
    outdir="./",
    ...
    #                 "fill" : ROOT.kOrange}
    colorBandPart = {"line": ROOT.kCyan + 2, "fill": ROOT.kCyan - 7}
    colorBandTot = {"line": ROOT.kOrange + 7, "fill": ROOT.kOrange - 3}
...
def getGlobalBin(ix, iy, nbinsX, binFrom0=True):
    ...
    if binFrom0:
        ...
        return (
            ix - 1 + (iy - 1) * nbinsX
        ) + 1  # trasform ix,iy in [1,N] to ix',iy' in [0,N-1], get global bin and sum 1 so that it starts from 1
...
def getEtaPtBinning(inputBins, whichBins="reco"):
    ...
    if whichBins not in ["reco", "gen"]:
        logger.error(
            "In function getEtaPtBinning(): whichBins must be 'reco' or 'gen'. Exit"
        )
        exit()
    ...
    if inputBins.startswith("file=") or re.match(".*binningPtEta.*.txt", inputBins):
        ...
        for x in content:
            if str(x).startswith(whichBins):
                ...
                continue
        etabinning = tmpbinning.split("*")[
            0
        ]  # this is like [a,b,c,...], and is of type string. We nedd to get an array
        ptbinning = tmpbinning.split("*")[1]
    else:
        etabinning = inputBins.split("*")[
            0
        ]  # this is like [a,b,c,...], and is of type string. We nedd to get an array
...
def unroll2Dto1D(
    h, newname="", cropNegativeBins=True, silent=False, invertUnroll=False
):
    nbins = h.GetNbinsX() * h.GetNbinsY()
    goodname = h.GetName()
    # h.SetName(goodname+"_2d")
    newh = ROOT.TH1D(
        f"{goodname}_unrolled" if not newname else newname,
        h.GetTitle(),
        nbins,
        0.5,
        nbins + 0.5,
    ...
    if cropNegativeBins:
        for ibin in range(1, nbins + 1):
            if newh.GetBinContent(ibin) < 0:
                if not silent:
                    logger.warning(
                        "unroll2Dto1D(): cropping to zero bin %d in %s (was %f)"
                        % (ibin, newh.GetName(), newh.GetBinContent(ibin))
                    )
...
def drawGraphCMS(
    ...
    for ig in range(0, nGraphs):
        if not useOriginalGraphStyle:
            ...
            if vecLineStyle:
                grList[ig].SetLineStyle(
                    vecLineStyle[ig] if ig < len(vecLineStyle) else vecLineStyle[-1]
                )
            if vecMarkerStyle:
                grList[ig].SetMarkerStyle(
                    vecMarkerStyle[ig]
                    if ig < len(vecMarkerStyle)
                    else vecMarkerStyle[-1]
                )
            else:
    ...
    if not skipLumi:
        if not drawLumiLatex:
            ...
            if lumi != None:
                latCMS.DrawLatex(0.745, 0.95, "%s fb^{-1} (13 TeV)" % lumi)
            else:
                latCMS.DrawLatex(0.845, 0.95, "(13 TeV)")
