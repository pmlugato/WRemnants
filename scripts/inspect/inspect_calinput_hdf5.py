...
def plot_muon_scale_systematics(histograms, outdir):
    for (dataset_name, hist_name), hsyst in sorted(histograms.items()):
        ...
        if "unc" not in hsyst.axes.name or "downUpVar" not in hsyst.axes.name:
            print(
                f"  Skipping {hist_name}: expected tensor axes 'unc' and 'downUpVar'"
            )
            continue
...
def main():
    ...
    parser.add_argument("input", help="Input HDF5 file from dimuon_resonances_calinput.py")
    parser.add_argument(
        "-o",
        "--outdir",
        default=None,
    ...
    outdir = Path(args.outdir) if args.outdir else infile.with_suffix("").parent / (
        infile.with_suffix("").name + "_plots"
    )
