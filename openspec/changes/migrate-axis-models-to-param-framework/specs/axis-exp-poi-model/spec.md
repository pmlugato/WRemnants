...
`rabbit/rabbit/param_models/param_model.py` that assigns per-(process, cell)
amplitude parameters and optionally grouped slope parameters, where cells are
defined by caller-specified cell axes. In `compute()` the model produces
...
- Set `npoi = n_procs * (n_cell + n_slope_groups)` where `n_cell` is the product
  ...
  set `npou = 0`. Parameter names (`self.params`) follow
  `lnAmpl_{procname}_{cell_label}` (one per cell) then
...
- In `compute(param, full=False)` lay out parameters per process as
  ...
  non-slope cell axes.
- Set `allowNegativeParam = True` (both parameters are unconstrained reals;
  `exp(lnAmpl + slope·x)` is always positive) and `is_linear = False`.
- Set `xparamdefault = tf.zeros([npoi])` so that the default prediction is a
...
- **THEN** `model.npoi == 224` (1 process × (112 lnAmpl + 112 slope), where
  112 = 8×7×2), `model.npou == 0`, parameter names begin with
  `lnAmpl_bkgExp_...` then `slope_bkgExp_...`, and `model.n_slope_groups == 112`
...
- **THEN** `model.npoi == 119` (1 process × (112 lnAmpl + 7 slope)),
  `model.n_slope_groups == 7`, and slope parameter names are
  `slope_bkgExp_bkmm_kaon_eta0` through `slope_bkgExp_bkmm_kaon_eta6`

#### Scenario: compute() produces flat unit rnorm at default parameters
- **WHEN** all parameters are 0.0 (xparamdefault)
